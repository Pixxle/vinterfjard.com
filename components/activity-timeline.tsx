...
export default function ActivityTimeline({ activityData }: ActivityTimelineProps) {
  ...
  if (!activityData) {
    return (
      <div className="rounded-md border border-yellow-600 bg-yellow-900/30 p-4">
        <p className="text-sm text-yellow-400">No GitHub activity data available right now.</p>
      </div>
    );
  }

  // Process and display real GitHub activity data
  const { commitContributionsByRepository, pullRequestContributions, issueContributions } =
    activityData;
              ...
              return (
                <div key={`commit-group-${index}`} className="flex">
                  <div className="mr-3 flex flex-col items-center">
                    ...
                    {!isLastItem && <div className="mt-2 h-full w-0.5 bg-gray-700"></div>}
                  </div>
                  <div className="flex-1">
                    <div className="flex items-center justify-between">
                      <h4 className="text-sm font-medium">
                        Created {commitCount} {formatPlural(commitCount, 'commit', 'commits')} in{' '}
                        {repoCount} {formatPlural(repoCount, 'repository', 'repositories')}
                      </h4>
                      ...
                      {Array.from(uniqueRepos).map(repoName => {
                        ...
                        return (
                          <div
                            ...
                            className="rounded-md border border-gray-700 bg-[#161b22] p-3"
                          >
                  ...
                  <div className="flex-1">
                    <div className="flex items-center justify-between">
                      <h4 className="text-sm font-medium">
                        ...
                        {formatPlural(group.items.length, 'pull request', 'pull requests')} across{' '}
                        {
                          new Set(
                            group.items.map(
                              item =>
                                (item.data as PullRequestContribution).pullRequest.repository
                                  .nameWithOwner
                            )
                          ).size
                        }{' '}
                        ...
                        return (
                          ...
                          >
                            <div className="flex items-center text-sm">
                              ...
                              />
                              <a href={pr.url} className="text-blue-400 hover:underline">
                                {pr.title}
                              </a>
            ...
            case 'issue':
              return (
                <div key={`issue-group-${index}`} className="flex">
                  <div className="mr-3 flex flex-col items-center">
                    <div className="flex h-8 w-8 items-center justify-center rounded-full bg-gray-800">
                      <GitIssueOpened className="h-4 w-4 text-gray-400" />
                    </div>
                    {!isLastItem && <div className="mt-2 h-full w-0.5 bg-gray-700"></div>}
                  </div>
                  <div className="flex-1">
                    <div className="flex items-center justify-between">
                      <h4 className="text-sm font-medium">
                        ...
                        {formatPlural(group.items.length, 'issue', 'issues')} across{' '}
                        {
                          new Set(
                            group.items.map(
                              item =>
                                (item.data as IssueContribution).issue.repository.nameWithOwner
                            )
                          ).size
                        }{' '}
                        {formatPlural(
                        ...
                        return (
                          ...
                          >
                            <div className="flex items-center text-sm">
                              ...
                              <div className="ml-auto flex items-center rounded-full bg-red-900 px-2 py-0.5 text-xs text-red-300">
                            ...
                            <div className="mt-2 text-sm">
                              <div className="flex items-center">
                                <GitIssueOpened
                                  className={`mr-2 h-4 w-4 ${
                                    issue.state === 'CLOSED' ? 'text-red-500' : 'text-green-500'
                                  }`}
                                />
                                <a href={issue.url} className="text-blue-400 hover:underline">
                                  {issue.title}
                                </a>
                              </div>
                            </div>
